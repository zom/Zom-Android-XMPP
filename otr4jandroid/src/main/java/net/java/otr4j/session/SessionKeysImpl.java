...
class SessionKeysImpl implements SessionKeys {
    ...
    private final static String LOG_TAG = "OtrSession";
    ...
    public void setLocalPair(KeyPair keyPair, int localPairKeyID) {
        ...
        if (DEBUG_ENABLED) Log.d(LOG_TAG,keyDescription + " current local key ID: " + this.getLocalKeyID());
        this.reset();
    }
    ...
    public void setRemoteDHPublicKey(DHPublicKey pubKey, int remoteKeyID) {
        ...
        if (DEBUG_ENABLED) Log.d(LOG_TAG,keyDescription + " current remote key ID: " + this.getRemoteKeyID());
        this.reset();
    }
    ...
    public void incrementSendingCtr() {
        if (DEBUG_ENABLED) Log.d(LOG_TAG,"Incrementing counter for (localkeyID, remoteKeyID) = (" + getLocalKeyID()
                      + "," + getRemoteKeyID() + ")");
    ...
    public byte[] getSendingAESKey() throws OtrException {
        ...
        buff.get(key);
        if (DEBUG_ENABLED) Log.d(LOG_TAG,"Calculated sending AES key.");
    ...
    public byte[] getReceivingAESKey() throws OtrException {
        ...
        ByteBuffer buff = ByteBuffer.wrap(h1);
        buff.get(key);
        if (DEBUG_ENABLED) Log.d(LOG_TAG,"Calculated receiving AES key.");
    ...
    public byte[] getSendingMACKey() throws OtrException {
        ...
        if (DEBUG_ENABLED) Log.d(LOG_TAG,"Calculated sending MAC key.");
        return sendingMACKey;
    }
    ...
    private BigInteger getS() throws OtrException {
        if (s == null) {
            s = new OtrCryptoEngineImpl().generateSecret(getLocalPair().getPrivate(),
                    getRemoteKey());
            if (DEBUG_ENABLED) Log.d(LOG_TAG,"Calculating shared secret S.");
        }
        return s;
    }
